...
const FOOTER_CONSTELLATION = ["presence", "press", "studios", "foundation", "guardian"] as const;
...
export default function ConstellationStrip() {
  return <footer className="relative z-10 mt-auto border-t border-mercury bg-carbon">
    <div className="border-b border-mercury px-6 py-2 text-center">
      <Link href="/" className="footer-system-status font-mono text-xs tracking-widest text-water">SYSTEM STATUS: ALL CHANNELS ACTIVE</Link>
    </div>
    <div className="mx-auto max-w-[1200px] px-6 py-6">
      <div className="flex flex-wrap items-stretch justify-center gap-x-3 gap-y-3 sm:gap-x-5">
        {FOOTER_CONSTELLATION.map((id) => { const pillar = PILLARS[id]; return <Link key={id} href={pillar.href} className="footer-constellation-link group flex min-w-[6.8rem] flex-col items-center justify-center gap-1 border border-transparent px-4 py-3 text-center" style={{ "--constellation-color": pillar.color } as CSSProperties}>
